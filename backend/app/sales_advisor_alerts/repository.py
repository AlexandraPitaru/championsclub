from typing import Optional, Tuple, List

from sqlalchemy import case, desc, func
from sqlmodel import Session, select

from app.models.user_alert import UserAlert


def get_sales_advisor_alerts_from_db(
    session: Session,
    user_id: int,
    priority: Optional[str] = None,
    is_read: Optional[bool] = None,
    alert_type: Optional[str] = None,
    limit: int = 10,
    offset: int = 0,
) -> Tuple[List[UserAlert], int, int]:
    filters = [UserAlert.user_id == user_id]

    if priority is not None:
        filters.append(func.lower(UserAlert.priority) == priority.lower())

    if is_read is not None:
        filters.append(UserAlert.is_read == is_read)

    if alert_type is not None:
        filters.append(func.lower(UserAlert.alert_type) == alert_type.lower())

    priority_order = case(
        (func.lower(UserAlert.priority) == "high", 1),
        (func.lower(UserAlert.priority) == "medium", 2),
        (func.lower(UserAlert.priority) == "low", 3),
        else_=4,
    )

    statement = (
        select(UserAlert)
        .where(*filters)
        .order_by(
            desc(func.date(UserAlert.created_at)),
            priority_order,
            desc(UserAlert.created_at),
        )
        .offset(offset)
        .limit(limit)
    )

    total_alerts = session.exec(
        select(func.count(UserAlert.alert_id)).where(UserAlert.user_id == user_id)
    ).one()

    if is_read is True:
        unread_alerts = 0
    else:
        unread_filters = [UserAlert.user_id == user_id, UserAlert.is_read == False]
        if priority is not None:
            unread_filters.append(func.lower(UserAlert.priority) == priority.lower())
        if alert_type is not None:
            unread_filters.append(func.lower(UserAlert.alert_type) == alert_type.lower())
        unread_alerts = session.exec(
            select(func.count(UserAlert.alert_id)).where(*unread_filters)
        ).one()

    return session.exec(statement).all(), total_alerts, unread_alerts
